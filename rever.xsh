...
def install_deps():
    ...
    conda create --name $TEMP_ENV python=3.8 --yes --quiet
    activate($TEMP_ENV)
...
def create_changelog():
    ...
    """
    loghub $GITHUB_ORG/$GITHUB_REPO -zr @($PROJECT + ' v' + $NEW_VERSION) -ilg type:feature "Features " -ilg type:enhancement "Enhancements" -ilg type:bug "Bugs fixed" -ilr "reso:completed" --no-prs
